                  ...
                  <div class="w-full">
                    ...
                    >
                      ...
                      <n-button
                        ...
                        rounded-4
                        :disabled="modalType === 'detail'"
                        @click="addTechnidal(index, formValue.sourceObjects[index]?.oid)"
                      >
                        <template #icon>
...
const { queryCharacterValueList, queryMModuleList } = useEnum()
...
const save = async (callback = null) => {
  formRef.value?.validate(async (errors) => {
    if (!errors) {
      ...
      data.value.forEach((item) => {
        ...
        tableArr.push({
          sourceValues: sourceValues.map((val, inx) => ({
            oid: val,
            value:
              columns.value[inx]?.valueEnum.find((itemValue) => itemValue.key === val)?.value ||
              val,
          })),
      ...
      const payload = { oid: handleOid.value, ...formObj, mappingValues: tableArr, type: 'mapping' }
