          ...
          <div class="form">
            ...
            >
              <n-grid :cols="24" :x-gap="24">
                  ...
                  <div class="w-full">
                    ...
                    >
                      <n-select
                        ...
                        placeholder="请选择"
                        :options="computedAllDesignCharacterList"
                        label-field="value"
                        value-field="key"
                      ...
                      <n-button
                        ...
                        w-32
                        rounded-4
                        :disabled="modalType === 'detail' || disabledAdd(item)"
                  ...
                  <div class="w-full">
                    ...
                    >
                      ...
                      <div ml-10 h-32 w-32 flex-shrink-0>
                        <n-button h-32 w-32 rounded-4 @click="addResult(index)">
                          <template #icon>
                            <the-icon type="custom" icon="addBtn" :size="14" />
                          </template>
                        </n-button>
                ...
                <n-form-item-gi :span="24" label="特征值" pt-20>
                  <n-button
                    attr-type="button"
                    type="primary"
                    ml-20
...
import { expressionComplianceCheck } from '~/src/api/feature'
import SvgIcon from '~/src/components/icon/SvgIcon.vue'
import useEnum from '~/src/hooks/useEnum'
import _ from 'lodash'
import {
  createIncludeExcludeRule,
  getIncludeExcludeRuleDetail,
  updateIncludeExcludeRule,
} from '~/src/api/config'
import { hasDuplicate, isEmpty } from '~/src/utils'
import {
  getGlobalConfigFeatureList,
  mutexRuleConditionFeature,
  mutexRuleConditionFeatureValueCombo,
} from '~/src/api/global'
...
const rulesStatus = ref('1') // 1 同选 2 互斥
const textareaState = ref(1) // 1 禁止修改 2 允许修改
const storageExpression = ref('')
const selectOid = ref('')
const saveState = ref(false)
const internalCarOid = ref('')
const modalLoading = ref(false)

const computedAllDesignCharacterList = computed(() => {
  const oidKey = formValue.value?.targetObjects.map((item) => item.oid)
  return allDesignCharacterList.value.filter((item) => !oidKey.includes(item.key))
})
const computedResultList = computed(() => {
  const oidKey = formValue.value?.sourceObjects.map((item) => item.oid)
  return resultList.value.filter((item) => !oidKey.includes(item.key))
})
...
const resultFeature = async (val, obj, index) => {
  ...
  const res = await mutexRuleConditionFeatureValueCombo({ oid: val, docOid: internalCarOid.value }) // 动态获取特征值枚举
  ...
  columns.value.splice(index, delteLength, {
    ...
    render(row, inx) {
      return h(NSelect, {
        ...
        onUpdateValue(v) {
          // const arr = characterList.filter((item) => v.includes(item.key)).map((item) => item.key)
          data.value[inx][val] = v
        },
      })
    },
...
const renderColums = async (sourceObjects = [], targetObjects = []) => {
  ...
  for (const item of sourceObjects) {
    const res = await mutexRuleConditionFeatureValueCombo({
      oid: item.oid,
      docOid: internalCarOid.value,
    }) // 动态获取特征值枚举
    sourceArr.push({
      title: '条件：' + (item.name || ''),
      key: item.oid,
      ...
      render(row, index) {
        return h(NSelect, {
          ...
          onUpdateValue(v) {
            // const arr = (res.data || []).filter((it) => v.includes(it.key)).map((it) => it.key)
...
const save = async (callbackFunction = null) => {
      ...
      data.value.forEach((item) => {
        ...
        tableArr.push({
          ...
          targetValues: targetValues.map((val, inx) => ({
            oid: isArray(val) ? val.join(',') : val,
            // value: columns.value[inx + 1]?.valueEnum.find((itemValue) => itemValue.key === val)
            //   ?.value,
          })),
        })
      })
      /* 判断特征值是否重复 */
      // const sourceValuesArr = tableArr.map((item) => item.sourceValues)
      // if (sourceValuesArr) {
      //   $message.warning('条件特征组合值不能相同')
      //   return
      // }
      const payload = {
        oid: selectOid.value,
        ...
        type: rulesStatus.value === '1' ? 'include' : 'exclude',
      }
      const apiUrl =
        modalType.value === 'edit' ? updateIncludeExcludeRule : createIncludeExcludeRule
 ...
 * @param {*} type
 * @param {*} row
 * @param {*} rulesStatus 1 同选 2 互斥
 * @param {*} internalCarOid 内部车型号oid
 * @return {*}
 */
const show = (type, row, _rulesStatus, internalOid) => {
  saveState.value = false
  modalType.value = type
  rulesStatus.value = _rulesStatus
  selectOid.value = row.oid
  internalCarOid.value = internalOid
  /* 判断是否获取详情 */
  if (type === 'detail' || type === 'edit') {
    fetchData(row.oid)
  }
  // fetchAllDesignCharacterList(row.oid)
  // fetchResultList(row.oid)
  fetchFeatureList()
...
const fetchData = async (oid) => {
  ...
  try {
    ...
    res = await getIncludeExcludeRuleDetail({ oid })
  } catch (error) {
  ...
  if (arrData.mappingValues.length > 0) {
    ...
    data.value = arrData.mappingValues.map((item, index) => {
      ...
      item.sourceValues.forEach((val, inx) => {
        sourceValues[arrData.sourceObjects[inx].oid] = val.oid.split(',')
      })
      /* 结果特征 */
      const targetValues = {}
...
const fetchAllDesignCharacterList = async () => {
  const res = await getGlobalConfigFeatureList({
...
const fetchFeatureList = async () => {
  const res = await mutexRuleConditionFeature({ oid: route.query.oid })
  allDesignCharacterList.value = res.data || []
  resultList.value = res.data || []
}
